...
export const GROUNDED_ABSTENTION_TEXT =
  "I do not have enough citation-backed evidence to answer reliably.";

const hasText = (value) => typeof value === "string" && value.trim().length > 0;

export const buildGroundedAbstention = ({
  answerText = "",
  claimSupport,
} = {}) => ({
  text: GROUNDED_ABSTENTION_TEXT,
  changed: String(answerText ?? "").trim() !== GROUNDED_ABSTENTION_TEXT,
  abstained: true,
  removedClaims: (claimSupport?.claims ?? [])
    .filter((claim) => !claim.supported && !claim.heading)
...
export const normalizeGroundedClaimSupportForHeadings = (claimSupport) => {
...
export const normalizeClaimSupportForHeadings =
...
const buildFinalizedText = ({ claimSupport, citations, nodes = [] }) => {
  ...
  if (supportedEvidenceClaims.length === 0) {
    return GROUNDED_ABSTENTION_TEXT;
  }
...
export const finalizeGroundedAnswer = ({
  answerText = "",
  citations = [],
  ...
  const normalizedClaimSupport =
    normalizeGroundedClaimSupportForHeadings(claimSupport);
...
export const finalizeAgentAnswer = finalizeGroundedAnswer;
