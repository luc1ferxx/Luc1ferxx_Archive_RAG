...
test("postgres task store persists scoped task snapshots and event records", async () => {
  ...
  const query = async (queryText, values = []) => {
    ...
    if (queryText.includes("FROM rag_tasks_test")) {
      const [userId, workspaceId, type, limit, offset] = values;
...
test("postgres task store list() appends parameterized LIMIT and OFFSET", async () => {
  const capturedQueries = [];
  const query = async (queryText, values = []) => {
    capturedQueries.push({ queryText, values });

    if (queryText.includes("INSERT INTO")) {
      const row = buildFakeTaskRow(values);
  ...
  await store.list({ accessScope, limit: 10, offset: 20 });
  ...
  capturedQueries.length = 0;
  await store.list({ accessScope, limit: 5000 });
  ...
  capturedQueries.length = 0;
  await store.list({ accessScope, limit: -5, offset: -10 });
  ...
  capturedQueries.length = 0;
  await store.list({ accessScope, limit: "abc", offset: null });
  ...
  capturedQueries.length = 0;
  await store.list({ accessScope, limit: "all" });
...
test("postgres task store list() uses parameterized LIMIT/OFFSET (never interpolated)", async () => {
  const capturedQueries = [];
  const query = async (queryText, values = []) => {
    capturedQueries.push({ queryText, values });

    if (queryText.includes("INSERT INTO")) {
      return { rowCount: 1, rows: [buildFakeTaskRow(values)] };
    }
