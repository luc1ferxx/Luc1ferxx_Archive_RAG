...
| AgentRAG | LLM/deterministic planner 可配置，执行前校验 access scope；支持 clarification gate、approval gate、白名单 skill chain、self-check、gap analysis、follow-up retrieval、finalizer、research_task/dossier 流程、agent task 产物交付，以及 contract-only connector/MCP adapter 层。 |
...
- Connector / MCP adapter 目前是 contract-only；默认不会加载真实外部工具，未注入 executor 的 connector capability 即使获得批准也不会执行。
