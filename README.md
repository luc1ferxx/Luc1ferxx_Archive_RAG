...
- `/chat` 会返回 `agentObservability`，按 skill 记录选中状态、耗时、citations、abstain、retry/follow-up、budget、执行 loop 和错误，feedback/eval 会保留这些指标。
...
2. Clarification gate：范围不清、没选文档、文档过多或证据不足时返回澄清问题，不硬答。
3. Task memory / working memory：记录本次 run 已查 query、已有证据 claims 和未解决 gaps。
4. Skill chaining：让 agent 能组合 `summarize_contract`、`risk_review`、`compare_documents`、`extract_timeline`。
5. Agent trace UI：在前端展示 skill 选择、检索 query、证据不足和 finalizer 删除的 claims。
