...
| arXiv 导入 | 上传后基于本地 profile keyphrase 和 relevance check 推荐相关 arXiv 论文，保存可稍后查看的推荐 snapshot，并写入通用 task log；用户勾选确认后按签名候选 token 下载，按 arXiv ID / PDF URL / title hash 去重并带 provenance 导入索引。 |
...
| AgentRAG | Planner、白名单 skill chain、self-check、gap analysis、follow-up retrieval、finalizer 和 scope-aware task log 已完成初版。 |
