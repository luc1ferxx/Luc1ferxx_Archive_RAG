...
| `server/rag/llmops-metrics.js` | 定义 LLMOps metric event contract，统一记录 completion、embedding 和 cross-encoder rerank 的 operation、stage、status、latency、公开 `modelRoute`、输入/输出规模、token usage、估算成本和 report-only latency SLO，不记录 prompt 原文或 secret。 |
...
- 事件只记录 operation、stage、status、latencyMs、公开 `modelRoute`、inputCharacters、outputCharacters、itemCount、token usage、token source、estimated cost、pricing source 和 latency SLO status；不记录 prompt 原文、chunk 原文、API key、secret ref value、transport 或 pricing rate。
...
- `server/evaluation/observability-report.js` 会从同一个 RAG observability JSONL 中汇总 LLMOps events，按 operation 和 model route 输出 count、平均延迟、error rate、token totals、estimated cost 和 SLO breach rate。

当前 LLMOps 薄切片已覆盖 usage/cost/SLO 的 report-only contract，但不做 annotation、alert 或 budget enforcement；后续这些能力应继续消费同一 metric event contract，而不是在各个模型调用点重复统计。
