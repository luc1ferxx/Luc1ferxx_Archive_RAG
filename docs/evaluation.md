...
5. `npm run quality:gate -- --fail-on-warn`
6. 如果存在 `server/data/feedback/feedback.jsonl`，再运行 `npm run eval:feedback` 和一次 quality gate

`Planner Real Provider Gate` workflow 通过 `workflow_dispatch` 和每日 schedule 触发。它不使用 conditional real step：会强制运行 mock planner eval、real planner eval，并执行 `npm run planner:gate -- --provider real --compare-provider mock --max-unexpected-fallback-rate=0 --max-divergence-count=0`。如果没有配置 `OPENAI_API_KEY` secret，real provider eval 会失败，从而暴露配置缺口。
