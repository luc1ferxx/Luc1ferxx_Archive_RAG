...
- `/chat` returns `agentObservability` with per-skill selected status, attempts, duration, citations, abstain, retry/follow-up, budget, execution loop, and error metrics. Preserve it when changing agent execution, feedback records, or feedback corpus metadata.
